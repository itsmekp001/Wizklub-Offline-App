...
Interpreter.prototype.initGlobalScope = function(scope) {
  ...
  var func = this.createNativeFunction(
      function(x) {throw EvalError("can not happen");}, false);
  func.eval = true;
...
"");
  ...
  wrapper = function(obj, prop, descriptor) {
    ...
    if (!obj.properties[prop] && obj.preventExtensions) {
      thisInterpreter.throwException(thisInterpreter.TYPE_ERROR,
          "can not define property '" + prop + "', object is not extensible");
    }
...
Interpreter.prototype.setProperty = function(obj, name, value, opt_descriptor) {
  ...
  if (!obj.isObject) {
    if (strict) {
      this.throwException(this.TYPE_ERROR, "can not create property '" + name +
                          "' on '" + obj + "'");
    }
    return;
  }
  if (this.isa(obj, this.STRING)) {
    ...
    if (name === 'length' || (!isNaN(n) && n < String(obj).length)) {
      // can not set length or letters on String objects.
      if (strict) {
  ...
  if (obj.preventExtensions && !(name in obj.properties)) {
    if (strict) {
      this.throwException(this.TYPE_ERROR, "can not add property '" + name +
...
Interpreter.prototype.populateScope_ = function(node, scope) {
  if (node['type'] === 'VariableDeclaration') {
    ...
    return;  // Expressions can not contain variable/function declarations.
  }
